...
void TestDomainBasicShape() {
  ...
  const stir_whir_gr::algebra::GRContext ctx(
      stir_whir_gr::algebra::GRConfig{.p = 2, .k_exp = 16, .r = 6});
  CHECK(stir_whir_gr::algebra::teichmuller_subgroup_size_supported(ctx, 9));
  ...
  const stir_whir_gr::Domain coset =
      stir_whir_gr::Domain::teichmuller_coset(ctx, coset_offset, 9);
...
void TestDomainRootOrderAndPowMap() {
  ...
  CHECK(stir_whir_gr::algebra::has_exact_multiplicative_order(ctx, domain.root(), 9));

  const stir_whir_gr::Domain pow3 = domain.pow_map(3);
  const stir_whir_gr::Domain pow9 = domain.pow_map(9);
...
void TestTeichmullerMembershipHelpers() {
  ...
  const stir_whir_gr::algebra::GRContext ctx(
      stir_whir_gr::algebra::GRConfig{.p = 2, .k_exp = 16, .r = 6});
  const auto domain = stir_whir_gr::Domain::teichmuller_subgroup(ctx, 9);
  const auto coset =
      stir_whir_gr::Domain::teichmuller_coset(ctx, ctx.teich_generator(), 9);

  auto two = ctx.with_ntl_context([&] { return ctx.one() + ctx.one(); });

  CHECK(stir_whir_gr::algebra::is_teichmuller_element(ctx, ctx.zero()));
  CHECK(stir_whir_gr::algebra::is_teichmuller_element(ctx, ctx.one()));
  CHECK(stir_whir_gr::algebra::is_teichmuller_element(ctx, ctx.teich_generator()));
  CHECK(stir_whir_gr::algebra::is_teichmuller_element(ctx, domain.element(4)));
  CHECK(stir_whir_gr::algebra::is_teichmuller_element(ctx, coset.element(2)));
  CHECK(!stir_whir_gr::algebra::is_teichmuller_element(ctx, two));
}
...
void TestDomainContainsHelper() {
  ...
  const stir_whir_gr::algebra::GRContext ctx(
      stir_whir_gr::algebra::GRConfig{.p = 2, .k_exp = 16, .r = 6});
  const stir_whir_gr::Domain subgroup = stir_whir_gr::Domain::teichmuller_subgroup(ctx, 9);
  const stir_whir_gr::Domain coset =
      stir_whir_gr::Domain::teichmuller_coset(ctx, ctx.teich_generator(), 9);
...
void TestDomainTeichmullerSubsetHelper() {
  ...
  const stir_whir_gr::algebra::GRContext ctx(
      stir_whir_gr::algebra::GRConfig{.p = 2, .k_exp = 16, .r = 6});
  const stir_whir_gr::Domain subgroup = stir_whir_gr::Domain::teichmuller_subgroup(ctx, 9);
  const stir_whir_gr::Domain teich_coset =
      stir_whir_gr::Domain::teichmuller_coset(ctx, ctx.teich_generator(), 9);
  ...
  const stir_whir_gr::Domain non_teich_coset =
      stir_whir_gr::Domain::teichmuller_coset(ctx, non_teich_unit, 9);
...
void TestDomainRejectsInvalidTeichmullerInputs() {
  ...
  try {
    (void)stir_whir_gr::Domain::teichmuller_subgroup(ctx, 10);
  ...
  try {
    (void)stir_whir_gr::Domain::teichmuller_coset(ctx, ctx.zero(), 9);
  } catch (const std::invalid_argument&) {
...
void TestLargeExtensionTeichGeneratorFallback() {
  ...
  const stir_whir_gr::algebra::GRContext ctx(
      stir_whir_gr::algebra::GRConfig{.p = 2, .k_exp = 16, .r = 162});
  CHECK(stir_whir_gr::algebra::teichmuller_subgroup_size_supported(ctx, 81));

  const auto offset = ctx.teich_generator();
  CHECK(ctx.is_unit(offset));

  const stir_whir_gr::Domain subgroup = stir_whir_gr::Domain::teichmuller_subgroup(ctx, 81);
  const stir_whir_gr::Domain coset = stir_whir_gr::Domain::teichmuller_coset(ctx, offset, 81);

  CHECK(stir_whir_gr::algebra::has_exact_multiplicative_order(ctx, subgroup.root(), 81));
  ctx.with_ntl_context([&] {
...
void TestMidExtensionPrimitiveTeichGeneratorPath() {
  ...
  const stir_whir_gr::algebra::GRContext ctx(
      stir_whir_gr::algebra::GRConfig{.p = 2, .k_exp = 16, .r = 18});
  CHECK(stir_whir_gr::algebra::teichmuller_subgroup_size_supported(ctx, 27));

  const auto offset = ctx.teich_generator();
  CHECK(ctx.is_unit(offset));

  const stir_whir_gr::Domain subgroup = stir_whir_gr::Domain::teichmuller_subgroup(ctx, 27);
  const stir_whir_gr::Domain coset = stir_whir_gr::Domain::teichmuller_coset(ctx, offset, 27);
...
void TestTeichGeneratorDeterministicAcrossFreshContexts() {
  ...
  for (int iteration = 0; iteration < 6; ++iteration) {
    const stir_whir_gr::algebra::GRContext ctx(
        stir_whir_gr::algebra::GRConfig{.p = 487, .k_exp = 2, .r = 1});
    CHECK(stir_whir_gr::algebra::teichmuller_subgroup_size_supported(ctx, 243));
    const auto generator_bytes = ctx.serialize(ctx.teich_generator());
    const auto root_bytes =
        ctx.serialize(stir_whir_gr::Domain::teichmuller_subgroup(ctx, 243).root());
...
void TestMidExtensionTeichGeneratorFallback() {
  ...
  const stir_whir_gr::algebra::GRContext ctx(
      stir_whir_gr::algebra::GRConfig{.p = 2, .k_exp = 16, .r = 54});
  CHECK(stir_whir_gr::algebra::teichmuller_subgroup_size_supported(ctx, 81));
