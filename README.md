...
- 已补 `scripts/search_params.py` 与 `scripts/run_search.sh`，可在 preset 基础上做参数枚举、聚合 size/time 结果，并输出 Top-K / Pareto 摘要。
...
- `search_params.py` 当前实现的是**工程型**参数搜索器：它复用现有 bench 二进制，给出候选全集、Top-K 与 Pareto 摘要；其中 `soundness_model = engineering-heuristic-v1` 仅表示当前启发式口径，后续可在形式化 soundness 推导补齐后替换；
