...
Wrapper around scripts/search_params.py with practical defaults.
...
Any additional flags are passed through to search_params.py.
Examples:
...
for arg in "$@"; do
  if [[ "$arg" == "-h" || "$arg" == "--help" ]]; then
    usage
    exec python3 "${SCRIPT_DIR}/search_params.py" --help
  fi
done
...
CMD=(
  python3 "${SCRIPT_DIR}/search_params.py"
  --preset "${PRESET_PATH}"
  --build-dir "${BUILD_DIR}"
  --combined-csv "${COMBINED_CSV}"
  --summary-md "${SUMMARY_MD}"
