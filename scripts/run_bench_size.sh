...
PRESET_PATH="${REPO_ROOT}/bench/presets/jia_micro_gr216_r162_size128.json"
...
Run Phase 6 proof-size estimator bench from a preset JSON plus optional overrides.

Options:
  --preset PATH         Preset JSON path (default: ${PRESET_PATH})
  --build-dir PATH      CMake build directory (default: ${BUILD_DIR})
  --output PATH         Output file path (default: results/size_<timestamp>.csv)
  --format FORMAT       text | csv | json (default: csv)
...
BENCH_BIN="${BUILD_DIR}/bench_proof_size_estimate"
if [[ ! -x "${BENCH_BIN}" ]]; then
...
if [[ -z "${OUTPUT_PATH}" ]]; then
  ...
  EXT="${FORMAT}"
  OUTPUT_PATH="${REPO_ROOT}/results/size_${TIMESTAMP}.${EXT}"
else
...
print(f"PRESET_POW={data.get('pow_bits', data.get('pow_bits_size', 22))}")
